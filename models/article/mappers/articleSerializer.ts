  ...
  return Article.create(
    {
      ...
      slug: article.slug,
      emoji: article.emoji,
      preamble: optionFrom(article.preamble ?? undefined),
      content: article.content,
      category: deserializeCategory(article.category),
      infographic: optionFrom(article.infographic ?? undefined),
      locale: deserializeLocale(article.locale),
      banner: deserializeExternalMultimediaAsset(article.banner),
      video: optionFrom(article.video ?? undefined).map(
        deserializeExternalMultimediaAsset
      ),
      references: optionFrom(article.references ?? undefined),
      bulletPoints: optionFrom(article.bulletPoints ?? undefined),
      referedArticles: article.referedArticles,
      seoMetadata: deserializeSeoMetadata(article.seoMetadata),
      description: article.description,
      metadata: optionFrom(article.metadata ?? undefined),
      createdAt: article.createdAt,
      publishedAt: article.publishedAt,
    },
    new Id(article.id)
